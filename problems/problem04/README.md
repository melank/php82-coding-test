...
3. shipping method `multiplier` を適用（`base * multiplier`）→ `feeBase`
4. 送料無料判定（`subtotal >= threshold` かつ `region != OKINAWA`）なら `feeBase` を 0
5. hazmat 商品が含まれれば `hazmatFee` を加算（送料無料でも加算）
6. 最終送料を int で返す（multiplier 計算後の端数は **切り上げ**）
...
| クラス | プロパティ / メソッド |
|--------|----------------------|
| `CartItem` | `price: int`, `quantity: int`, `hazmat: bool` |
| `Cart` | `items: list<CartItem>`, `subtotal(): int` |
| `Destination` | `regionCode: string` |
| `ShippingMethod` | enum: `Standard`, `Express` |
| `PricingPolicy` | `baseFees`, `multipliers`, `freeShippingThreshold`, `hazmatFee` |
| `ShippingService` | `calculate(...)` |
